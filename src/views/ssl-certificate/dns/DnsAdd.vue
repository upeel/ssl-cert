...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      try {
        const response = await axios.post(`${process.env.VUE_APP_API_BASE_URL}/api/dns`, this.dns)
        await this.$router.push({ name: 'dns-list' }, () => {
          this.$toast({
            component: ToastificationContent,
            props: {
              title: 'DNS Added Successfully',
              icon: 'CheckIcon',
              variant: 'success',
            },
          })
